...
function addTaskToDay(){const jobId=$('taskJob').value,title=$('taskTitle').value.trim(); if(!title)return; const ev=state.events.find(e=>e.date===selectedDate&&e.jobId===jobId) || state.events.find(e=>e.date===selectedDate); state.tasks.push({id:uid(),eventId:ev?ev.id:null,jobId,date:selectedDate,title,done:false}); save(); openDay(selectedDate);}
function toggleTask(id){const t=state.tasks.find(x=>x.id===id); t.done=!t.done; save(); openDay(selectedDate);}
